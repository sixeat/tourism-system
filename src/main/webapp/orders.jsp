...
function orderDateText(o){if(o.orderType==='HOTEL'){const start=fmtDate(o.checkInDate||o.useDate);const end=fmtDate(o.checkOutDate);return '入住/退房：'+start+' 至 '+end;}return '游玩日期：'+fmtDate(o.visitDate||o.useDate)+(o.quantity?' · 数量 '+o.quantity+' 张':'');}
...
function renderOrders(){document.querySelectorAll('[data-type]').forEach(btn=>btn.classList.toggle('active',btn.dataset.type===currentType));const list=filteredOrders();orders.innerHTML=list.map(o=>'<div class="card order-card"><div class="order-head"><div><div class="title">'+typeLabel(o.orderType)+' #'+o.id+'</div><div class="muted">'+e(o.itemName||'-')+'</div></div><span class="order-badge '+statusClass(o.orderStatus)+'">'+statusLabel(o.orderStatus)+'</span></div><div class="order-meta"><div class="muted">金额：<span class="price">&yen;'+Number(o.totalAmount||0).toFixed(2)+'</span></div><div class="muted">'+e(orderDateText(o))+'</div><div class="muted">创建时间：'+fmtDate(o.createTime)+'</div></div><div class="action-row">'+(canOperate(o)?'<button class="primary order-pay" data-order-type="'+e(o.orderType)+'" data-order-id="'+o.id+'">支付订单</button><button class="secondary order-cancel" data-order-type="'+e(o.orderType)+'" data-order-id="'+o.id+'">取消订单</button>':'<span class="secondary">已处理</span>')+'</div></div>').join('')||'<div class="empty-order"><div><strong>暂无订单</strong><p>去预订酒店或门票，提交后这里会立即显示。</p></div></div>';orders.querySelectorAll('.order-pay').forEach(btn=>btn.onclick=()=>payOrder(btn.dataset.orderType,Number(btn.dataset.orderId)));orders.querySelectorAll('.order-cancel').forEach(btn=>btn.onclick=()=>cancelOrder(btn.dataset.orderType,Number(btn.dataset.orderId)));}
